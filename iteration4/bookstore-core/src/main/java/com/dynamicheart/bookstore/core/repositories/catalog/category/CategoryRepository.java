...
public interface CategoryRepository extends JpaRepository<Category, Long>, CategoryRepositoryCustom {
	...
	@Query("select c from Category c left join fetch c.descriptions cd join fetch cd.language cdl where cd.name like ?1 and cdl.id=?2 order by c.sortOrder asc")
